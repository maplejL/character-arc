...
3. ~~**写作日志结构化聚合**~~ ✅（2026-07-19 完成）：新增 `shared/quality-pitfalls.ts`（server/electron 双副本）。每章收敛结束后把 audit/repair issue 按 severity 取前 8 条、按 category 计数写入 `knowledgeDocuments`（`sourceLabel: 'quality-issue-journal'`，独立于 writing-journal 标签，不挤占 memo 日志位；无 issue 不落文档）；终检未过早退路径同样落库。两侧 `buildMemoBaseContext` 用 `aggregateQualityPitfalls` 全项目计数取 Top5（各留最近 2 条样例）注入 memo 上下文，`chapter-memo` 渲染「近期高频坑」区块且 doNotDo 契约第④条要求覆盖与本章相关的反复问题。
4. ~~**批次结束时的整卷复盘**~~ ✅（2026-07-19 完成）：新增 `volume-batch-review` 任务（`electron/main/ai/tasks/volume-batch-review.ts`）。worker 在 run 标记 completed 之后执行：汇总本 run 完成章节（标题+摘要+300 字节选，至多 20 章）做「阶段摘要 + 批次级 risks」（只看跨章层面：伏笔遗忘/状态冲突/主线失焦/节奏塌陷），结果以 `sourceLabel: 'volume-review'` 写入 `knowledgeDocuments`（metadata 带 runId/volumeId/chapterIds/risks）；失败仅 `run-warning`，不影响 run 完成状态。
